...
// URL Reconstruction Start
  //int Authority_IP = authority.ip;
  
  char *consensusUrl[128], *consensusPort = (char)authority.clearnet_port;
  char consensusBuffer[64], consensusUrlPrefix[100] = "http://", *url = "/tor/status-vote/current/consensus";
  int urlRewrite = snprintf(consensusUrl, sizeof consensusUrl, "http://%s:%d%s", authority.ip, authority.clearnet_port,url);

  strcat(consensusBuffer,consensusUrlPrefix);
  sprintf(authority_port, "%d", authority.ip);
  strcat(consensusBuffer,authority_port);

  consensus_url = consensusUrl;

// URL Reconstruction End

  sprintf(authority_port, "%d", authority.clearnet_port);
  //  strcat(authority.ip,":");
    //int peanut = printf("http://%s\%s%d%s\n",authority.ip,":",authority.clearnet_port,"/tor/status-vote/current/consensus");
    //consensus_url = peanut;
